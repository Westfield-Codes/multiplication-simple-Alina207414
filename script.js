...
let score = 0;
let question = setUp();
for (let question = 1;question <= question;question++) {
...
function showStats(score, questions) {
    ...
    while (tables == true) {
     tables = confirm("Do you want study " + more + " tables");
    if (tables == true) {
        factor  = prompt("Show table wich factor?");
        showTable(factor);
         more = "more";
        }   
...
function showTable(factor) {
    ...
    for (let line = low; line <= Highlight; line++){
